...
import CreateExpenseForm, { formInputs } from "../CreateExpenseForm/CreateExpenseForm";
...
const ExpenseSettlementPage = () => {
  ...
  const { data: group, error: groupError } = useApi<Group>(`${import.meta.env.VITE_API_BASE_URL}/groups/${groupName}`);
  const {
    data: settlements,
    error: settlementsError,
    refetch: refetchSettlements,
    postData: postExpense,
  } = useApi<Settlement[]>(`${import.meta.env.VITE_API_BASE_URL}/settlements/${groupName}`);
  ...
  return (
    <VStack m={6} w={"full"} maxW={"600px"} spacing={6}>
      ...
      {settlements && <SettlementList settlements={settlements}></SettlementList>}
    </VStack>
  );
};
