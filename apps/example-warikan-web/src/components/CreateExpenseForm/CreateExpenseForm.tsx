...
const CreateExpenseForm: React.FC<CreateExpenseFormProps> = ({ group, onSubmit }) => {
  const {
    register,
    handleSubmit,
    reset,
      ...
      <form onSubmit={handleSubmit(onSubmitFrom)} style={{ width: "60%" }}>
        <VStack>
          <FormControl mb={5} isInvalid={Boolean(errors.expenseName)}>
            ...
            <FormErrorMessage>{errors.expenseName && errors.expenseName.message}</FormErrorMessage>
          </FormControl>
          ...
          <Button colorScheme="green" isLoading={isSubmitting} type="submit" w={"full"} m={5}>
            支出を登録
          </Button>
        </VStack>
      </form>
